...
<template><aside class="navigation-panel"><header><div><p class="eyebrow">Browse</p><h2>Project contents</h2></div><button class="icon-button" @click="emit('close')">×</button></header><div class="navigation-groups"><section v-for="[title,items] in groups" :key="title"><h3>{{ title }}</h3><button v-for="item in items" :key="item.id" @click="emit('navigate',item.id)">{{ item.title }}</button></section></div></aside></template>
